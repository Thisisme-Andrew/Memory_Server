...
export const getUserByID = async (userID) => {
  try {
    ...
    const user = {userID: response.id, firstName: response.firstName, lastName: response.lastName, email: response.email};
...
export const login = async (email, password) => {
  ...
  try {
    ...
    if(!response) {
      throw "Couldn't find user with those credentials.";
    }
    const user = {userID: response.id, firstName: response.firstName, lastName: response.lastName, email: response.email};
    console.log("user " + response.id + " verified");
    return user;
  } catch (err) {
